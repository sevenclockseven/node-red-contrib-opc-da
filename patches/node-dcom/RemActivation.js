...
class RemActivation extends NdrObject {
    ...
    constructor(clsid, interfaces) {
        ...
        // Map<String(iid), InterfacePointer> — stores ALL returned interface pointers
    ...
    read(ndr) {
        ...
        // Match with requested interfaces array to build IID → InterfacePointer map
        for (let i = 0; i < arrayObjs.length && i < this.interfaces.length; i++) {
            if (arrayObjs[i] != null) {
                let iid = this.interfaces[i].toString().toUpperCase();
                this.interfaceMap.set(iid, arrayObjs[i]);
            }
        }
    ...
    /**
     * @return {Map<string, InterfacePointer>}
     */
    getInterfaceMap() {
