...
class Responses
{
    ...
    lmHash (password)
    {
        ...
        let des = Crypto.createCipheriv('des-ecb', Buffer.from(lowKey), Buffer.alloc(0));
        des.update(magicConstant);
        let lowHash = [...des.final()];
        //aux = des.final();

        des = Crypto.createCipheriv('des-ecb', Buffer.from(highKey), Buffer.alloc(0));
        des.update(magicConstant)
        let highHash = [...des.final()];

        let lmHash = new Array();
        lmHash = lmHash.concat(lowHash);
        lmHash = lmHash.concat(highHash);
    ...
    lmResponse (hash, challenge)
    {
        ...
        let des = Crypto.createCipheriv("des-ecb", lowKey, Buffer.alloc(0));
        let lowResponse = des.update(challenge);
        des.final();

        des = Crypto.createCipheriv("des-ecb", middleKey, Buffer.alloc(0));
        let middleResponse = des.update(challenge)
        des.final();

        des = Crypto.createCipheriv("des-ecb", highKey, Buffer.alloc(0));
    ...
    lmv2Response (hash, clientData, challenge)
    {
        ...
        let aux_i = 0;
        while (aux.length > 0) data.splice(aux_i++, 1, aux.shift());
        ...
        lmv2Response = lmv2Response.concat(mac);
        lmv2Response = lmv2Response.concat(clientData);
    ...
    createBlob (targetInformation, clientNonce)
    {
        ...
        blob = blob.concat(blobSignature);
        blob = blob.concat(reserved);
        blob = blob.concat(timestamp);
        blob = blob.concat(clientNonce);
        blob = blob.concat(unknown1);
        blob = blob.concat(targetInformation);
        blob = blob.concat(unknown2);
    ...
    hmacMD5 (data, key)
    {
        ...
        let ipad_content = Buffer.concat([ipad, Buffer.from(data)], 64 + data.length);
        let hmac1 = Crypto.createHmac('md5', Buffer.alloc(0));
        hmac1.update(ipad_content);
        let intermediate = hmac1.digest();

        let opad_content = Buffer.concat([opad, intermediate], opad.length + intermediate.length);
        let hmac2 = Crypto.createHmac('md5', Buffer.alloc(0));
        hmac2.update(opad_content);
        return hmac2.digest();
    }
