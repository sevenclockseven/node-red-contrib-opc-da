...
class Type3Message extends NtlmMessage {
        ...
        case 1:
          if (!this.getFlag(NtlmFlags.NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY)) {
            ...
            if (this.getFlag(NtlmFlags.NTLMSSP_NEGOTIATE_KEY_EXCH)) {
              ...
              // RC4 with createCipheriv: key is used directly, no IV needed
              // RC4 is a stream cipher, so we use the key as-is with a zero-length IV
              let arcfour = Crypto.createCipheriv('rc4', Buffer.from(this.masterKey), Buffer.alloc(0));
  ...
  parse(material){
    ...
    for (let index = 0; index < 8; index++) {
      ...
      let domainBytes = this.readSecurityBuffer(material, pos);
