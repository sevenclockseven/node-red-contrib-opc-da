...
class RemUnknown extends NdrObject {
     ...
     */
    read(ndr) {
        // HEX DUMP: Log the 13-byte stub to understand the server response
        try {
            let buf = ndr.buf;
            if (buf && buf.buf && buf.buf.length < 100) {
                let hexStr = Buffer.from(buf.buf.slice(0, buf.buf.length)).toString('hex');
                console.log("[OPC-DA] RemUnknown read buffer (" + buf.buf.length + " bytes): 0x" + hexStr);
            }
        } catch(_) {}
